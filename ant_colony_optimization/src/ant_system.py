...
class KNearestAntSystem:
    def __init__(
        self,
        distance_matrix: np.ndarray,
        k_near_distance: np.ndarray,
        k_near_cities: np.ndarray,
        alpha: float,
        beta: float,
        evaporation_rate: float,
        n_ants: int,
        ...
        distance_matrix : np.ndarray
            ...
            the distance between city i and city j
        k_near_distance : np.ndarray
            Array of shape (n_cities, k) containing the distance \
            between the k nearest cities.
        k_near_cities : np.ndarray
            Array of shape (n_cities, k) containing the k nearest \
            cities.
        alpha : float
            Pheromone influence, alpha >= 0
        beta : float
        ...
        """
        self.distance_matrix = distance_matrix
        self.k_near_distance = k_near_distance
        self.k_near_cities = k_near_cities
        self.alpha = alpha
        self.beta = beta
        self.n_ants = n_ants
        self.evaporation_rate = evaporation_rate
        ...
        self.cities = np.arange(distance_matrix.shape[0])  # [0, 1, 2, ..., n_cities]
        self.pheromone = np.ones(distance_matrix.shape)
        self.k_near_pheromone = np.ones(k_near_distance.shape)
        self.best_solution: np.ndarray = None
        self.best_solution_cost: float = np.inf
        self.rng = rng
    ...
    def next_city(self, ant: int, current_city_index: int) -> int:
        ...
        visited_cities = self.tabu_list[ant, :current_city_index]
        current_city = visited_cities[-1]
        unvisited_cities_idx = [
            i
            for i in range(self.k_near_cities.shape[1])
            if self.k_near_cities[current_city][i] not in visited_cities
        ]
        if len(unvisited_cities_idx) > 0:
            pheromone = self.k_near_pheromone[current_city][unvisited_cities_idx]
            heuristic = 1 / self.k_near_distance[current_city][unvisited_cities_idx]
            probabilities = (pheromone**self.alpha) * (heuristic**self.beta)
            probabilities = probabilities / probabilities.sum()
            next_city_idx = np.random.choice(unvisited_cities_idx, p=probabilities)
            return self.k_near_cities[current_city][next_city_idx]

        # If all the k nearest cities have been visited, visit the other cities
        else:
            unvisited_cities = np.setdiff1d(self.cities, visited_cities)
            pheromone = self.pheromone[current_city, unvisited_cities]
            heuristic = self.distance_matrix[current_city, unvisited_cities]
    ...
    def evaporation(self) -> None:
        """Pheromone evaporation."""
        self.pheromone *= 1 - self.evaporation_rate
        self.k_near_pheromone *= 1 - self.evaporation_rate
    ...
    def reinforcement(self, solution: np.ndarray) -> None:
        ...
        next_cities = solution[1:]
        self.pheromone[current_cities, next_cities] += 1 / solution_cost
        for current, next in zip(current_cities, next_cities):
            next_idx = np.where(self.k_near_cities[current] == next)[0]
            if len(next_idx) > 0:
                self.k_near_pheromone[current, next_idx[0]] += 1 / solution_cost
